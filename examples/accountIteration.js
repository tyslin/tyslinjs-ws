...
// let wsString = "wss://eu-west-fra.district1.io:9090";
            ...
            try {
                lookupAccounts(startChar, accountLimit).then(accounts => {
                        accounts.forEach((account) => {
                            if ("balances" in account[1]) {
                                for (let i = 0; i < account[1].balances.length; i++) {
                                    if (account[1].balances[i].asset_type === assetID
                        ...
                        if (accounts.length === accountLimit && iterations < maxIter) {
                            iterations++;
                            console.log("Starting new iteration, so far found", Object.keys(holders).length, "holders");
